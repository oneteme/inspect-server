...
public class JoinConstant {
    ...
    public static JoinBuilder restRequestJoins(String name) {
        return switch (name) {
            case EXCEPTION_JOIN-> c-> new ViewJoin[] { leftJoin(EXCEPTION.view(), REST_REQUEST.column(ID).eq(EXCEPTION.column(PARENT))) };
            default -> null;
        };
    }
    ...
    public static JoinBuilder localRequestJoins(String name) {
        return switch (name) {
            case EXCEPTION_JOIN -> c-> new ViewJoin[] { leftJoin(EXCEPTION.view(), LOCAL_REQUEST.column(ID).eq(EXCEPTION.column(PARENT)), EXCEPTION.column(TYPE).eq(RequestType.LOCAL.name())) };
            default -> null;
        };
    }
    ...
    public static JoinBuilder databaseStageJoins(String name) {
        return switch (name) {
            case EXCEPTION_JOIN -> c-> new ViewJoin[] { leftJoin(EXCEPTION.view(), DATABASE_STAGE.column(PARENT).eq(EXCEPTION.column(PARENT)), DATABASE_STAGE.column(ORDER).eq(EXCEPTION.column(ORDER)), EXCEPTION.column(TYPE).eq(RequestType.JDBC.name())) };
            default -> null;
        };
    }
    ...
    public static JoinBuilder ftpStageJoins(String name) {
        return switch (name) {
            case EXCEPTION_JOIN -> c-> new ViewJoin[] { leftJoin(EXCEPTION.view(), FTP_STAGE.column(PARENT).eq(EXCEPTION.column(PARENT)), FTP_STAGE.column(ORDER).eq(EXCEPTION.column(ORDER)), EXCEPTION.column(TYPE).eq(RequestType.FTP.name())) };
            default -> null;
        };
    }
    ...
    public static JoinBuilder smtpStageJoins(String name) {
        return switch (name) {
            case EXCEPTION_JOIN -> c-> new ViewJoin[] { leftJoin(EXCEPTION.view(), SMTP_STAGE.column(PARENT).eq(EXCEPTION.column(PARENT)), SMTP_STAGE.column(ORDER).eq(EXCEPTION.column(ORDER)), EXCEPTION.column(TYPE).eq(RequestType.SMTP.name())) };
            default -> null;
        };
    }
    ...
    public static JoinBuilder ldapStageJoins(String name) {
        return switch (name) {
            case EXCEPTION_JOIN -> c-> new ViewJoin[] { leftJoin(EXCEPTION.view(), LDAP_STAGE.column(PARENT).eq(EXCEPTION.column(PARENT)), LDAP_STAGE.column(ORDER).eq(EXCEPTION.column(ORDER)), EXCEPTION.column(TYPE).eq(RequestType.LDAP.name())) };
            default -> null;
        };
    }
}
